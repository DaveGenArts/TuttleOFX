...
int VideoFFmpegWriter::start( )
{
	if( !_avFormatOptions )
	{
		...
		{
			TUTTLE_LOG_WARNING( "ffmpegWriter: output context allocation failed" );
			return false;
		}
		_ofmt = _avFormatOptions->oformat;
		TUTTLE_LOG_WARNING( "ffmpegWriter: " << std::string(_ofmt->name) << " format selected" );
	}
	...
	if( !_stream )
	{
		...
		TUTTLE_LOG_WARNING( "ffmpegWriter: " << std::string(_videoCodec->name) << " codec selected" );
...
//		TUTTLE_COUT_VAR2( _videoCodecName, _videoPresetName );
		...
		if ( !pixfmt_allowed )
		{
			...
			TUTTLE_LOG_WARNING( "ffmpegWriter: pixel format " << av_get_pix_fmt_name(_out_pixelFormat) << " not available in codec" );
			_out_pixelFormat = _videoCodec->pix_fmts[0];
			TUTTLE_LOG_WARNING( "ffmpegWriter: auto-selecting " << av_get_pix_fmt_name(_out_pixelFormat) );
		}
		...
		if( error )
		{
			TUTTLE_LOG_WARNING( ffmpegError_toString( error) );
			BOOST_THROW_EXCEPTION( exception::Format()
...
int VideoFFmpegWriter::execute( boost::uint8_t* const in_buffer, const int in_width, const int in_height, const PixelFormat in_pixelFormat )
{
	...
	else
	{
		...
		if( ret < 0 )
		{
			TUTTLE_LOG_WARNING( "ffmpegWriter: error writing packet to file" );
			TUTTLE_LOG_WARNING( getErrorStr(ret) );
			return false;
		}
...
void VideoFFmpegWriter::optionSet( const EAVParamType& type, const AVOption& opt, bool& value )
{
	...
	if( error )
		TUTTLE_LOG_WARNING( "FFMpeg writer: " << ffmpegError_toString( error ) << " : " << opt.name << " ( " << ( value ? "True" : "False" ) << " )" );
}
...
void VideoFFmpegWriter::optionSet( const EAVParamType& type, const AVOption &opt, bool& value, std::string& valueToSetFlag )
{
	...
	if( error )
		TUTTLE_LOG_WARNING( "FFMpeg writer: " << ffmpegError_toString( error ) << " : " << valueToSetFlag << " ( " <<  opt.name << " = " << ( value ? "True" : "False" ) << " )" );
}
...
void VideoFFmpegWriter::optionSet( const EAVParamType& type, const AVOption& opt, int& value )
{
	...
	if( error )
		TUTTLE_LOG_WARNING( "FFMpeg writer: " << ffmpegError_toString( error ) << " : " << opt.name << " ( " << value << " )" );
}
...
void VideoFFmpegWriter::optionSet( const EAVParamType& type, const AVOption &opt, int &valueNum, int& valueDen )
{
	...
	if( error )
		TUTTLE_LOG_WARNING( "FFMpeg writer: " << ffmpegError_toString( error ) << " : " << opt.name << " ( " << valueNum << "/" << valueDen<< " )" );
}
