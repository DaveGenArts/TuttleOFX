...
int convertIntWithOptionalUnit( const std::string& param, const std::string& stringValue )
{
	...
	if( std::isdigit( stringValue[ stringValue.length()-1 ] ) )
	{
		...
		{
			TUTTLE_LOG_WARNING( "FFMpeg writer: parameter " << param << " can't convert the value: " << stringValue );
		}
	}
	else
	{
		...
		switch( unit )
		{
			...
			case 'G':
				...
				break;
			default:
				TUTTLE_LOG_WARNING("FFMpeg writer: unknown unit " << unit << " for parameter " << param );
				break;
		}
	}
	return intValue;
...
void FFMpegWriterPlugin::setParameters( const PresetParameters& parameters )
{
	BOOST_FOREACH( PresetParameters::value_type param, parameters )
	{
		//TUTTLE_COUT_VAR2( param.first, param.second );
			if( paramExists( param.first ) )
			{
				...
				switch( getParamType( param.first ) )
				{
					...
					case OFX::eInt2DParam:
					{
						...
						{
							TUTTLE_LOG_WARNING("FFMpeg writer: unable to set an 2D Int value with " << param.second.size() << " parameters." );
						}
						break;
					}
					case OFX::eInt3DParam:
					{
						...
						{
							TUTTLE_LOG_WARNING("FFMpeg writer: unable to set an 3D Int value with " << param.second.size() << " parameters." );
						}
						break;
					}
					case OFX::eDoubleParam:
					{
						...
						{
							TUTTLE_LOG_WARNING( "FFMpeg writer: parameter " << param.first << " can't convert the value: " << value );
						}
						break;
					}
					case OFX::eDouble2DParam:
					{
						...
						if( param.second.size() == 2 )
						{
							...
							{
								TUTTLE_LOG_WARNING( "FFMpeg writer: parameter " << param.first << " can't convert the value: [" << param.second.at( 0 ) << ", " << param.second.at( 1 ) << "]" );
							}
						}
						else
						{
							TUTTLE_LOG_WARNING("FFMpeg writer: unable to set an 2D Double value with " << param.second.size() << " parameters." );
						}
						break;
					}
					case OFX::eDouble3DParam:
					{
						...
						if( param.second.size() == 3 )
						{
							...
							{
								TUTTLE_LOG_WARNING( "FFMpeg writer: parameter " << param.first << " can't convert the value: [" << param.second.at( 0 ) << ", " << param.second.at( 1 ) << ", " << param.second.at( 2 ) << "]" );
							}
						}
						else
						{
							TUTTLE_LOG_WARNING("FFMpeg writer: unable to set an 3D Double value with " << param.second.size() << " parameters." );
						}
						break;
					}
					case OFX::eRGBParam:
					{
						...
						{
							TUTTLE_LOG_WARNING("FFMpeg writer: unable to set an RGB value with " << param.second.size() << " parameters." );
						}
						break;
					}
					case OFX::eRGBAParam:
					{
						...
						if( param.second.size() == 4 )
						{
							...
							{
								TUTTLE_LOG_WARNING( "FFMpeg writer: parameter " << param.first << " can't convert the value: [" << param.second.at( 0 ) << ", " << param.second.at( 1 ) << ", " << param.second.at( 2 ) << ", " << param.second.at( 3 ) << "]" );
							}
						}
						else
						{
							TUTTLE_LOG_WARNING("FFMpeg writer: unable to set an RGBA value with " << param.second.size() << " parameters." );
						}
						break;
					}
					...
					case OFX::eChoiceParam:
					{
						...
						if( index > -1 )
							...
							TUTTLE_LOG_WARNING( "FFMpeg writer: unable to find value " << value << " for parameter " << param.first );
						break;
					}
			...
			{
				TUTTLE_LOG_WARNING( "FFMpeg writer: parameter " << param.first << " not exist." );
			}
	}
}
